...
-- Remove a specific announcement from the queue by GlobalID
local function removeFromQueue(globalId)
	for i, ann in ipairs(announcementQueue) do
		if ann.GlobalID == globalId then
			table.remove(announcementQueue, i)
			break
		end
	end
end
	...
	task.spawn(function()
		...
		if plr and plr.Parent then
			...
			if joinedCount > 0 then
				noti.createNotification(plr, "🎉 "..joinedCount.." player(s) joined from your announcement!", 5)
